...
로컬에 설치된 CLI 명령어(`gh`, `kubectl`, `kaf` 등)를 clip 게이트웨이에 등록합니다.
...
# prepend args: clip kaf 실행 시 항상 -c my-cluster 삽입
clip add kaf kaf --args "-c,my-cluster"
```
...
clip kaf topic list --dry-run
# kaf -c my-cluster topic list
```
